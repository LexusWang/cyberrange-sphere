#!/usr/bin/env bash
# ============================================================================
# Heartbleed Lab — Attacker Machine Setup
# ============================================================================
# Installs all dependencies needed to run heartbleed_attack.py on a minimal
# Kali image (SPHERE cyber range).
#
# The Heartbleed exploit is implemented in pure Python (raw sockets), so no
# external exploit frameworks (e.g. Metasploit) are needed.
#
# Usage:  sudo bash setup_heartbleed.sh
# ============================================================================
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  Heartbleed Lab — Attacker Machine Setup             ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
PACKAGES=(
    ...
    # TLS inspection
    openssl
    # Optional TLS scanner
    sslscan
    # SSH with password (post-exploitation)
    sshpass
...
ATTACK_SCRIPT="${SCRIPT_DIR}/../heartbleed_lab/heartbleed_attack.py"

if [[ -f "${ATTACK_SCRIPT}" ]]; then
    info "Copying attack script to ${REAL_HOME}/heartbleed_attack.py"
    cp "${ATTACK_SCRIPT}" "${REAL_HOME}/heartbleed_attack.py"
    chown "${REAL_USER}:${REAL_USER}" "${REAL_HOME}/heartbleed_attack.py"
    chmod +x "${REAL_HOME}/heartbleed_attack.py"
    ok "Attack script copied"
else
    warn "Attack script not found at ${ATTACK_SCRIPT}"
    info "You can manually copy heartbleed_lab/heartbleed_attack.py to the attacker"
fi
...
TOOLS=(
    ...
    "curl:HTTP client"
    "nmap:Port scanner (with ssl-heartbleed NSE script)"
...
# Check nmap ssl-heartbleed NSE script
NSE_SCRIPT="/usr/share/nmap/scripts/ssl-heartbleed.nse"
if [[ -f "${NSE_SCRIPT}" ]]; then
    echo -e "    ${GREEN}✔${NC}  ssl-heartbleed.nse  ${CYAN}— Nmap Heartbleed detection script${NC}"
else
    echo -e "    ${RED}✘${NC}  ssl-heartbleed.nse  ${YELLOW}— Nmap Heartbleed detection script (MISSING)${NC}"
    ALL_OK=false
fi
...
echo -e "    ${YELLOW}cd ~ && python3 heartbleed_attack.py${NC}"
echo ""
