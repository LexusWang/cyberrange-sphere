#!/usr/bin/env bash
# ============================================================================
# Redis Unauthorized Access Lab — Attacker Machine Setup
# ============================================================================
# Installs all dependencies needed to run redis_attack.py on a minimal
# Kali image (SPHERE cyber range).
#
# Usage:  sudo bash setup_redis.sh
# ============================================================================
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  Redis Unauthorized Access Lab — Attacker Setup      ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
PACKAGES=(
    ...
    python3-pip
    python3-venv
    # Redis CLI (redis-tools provides redis-cli)
    redis-tools
    # Port scanner
    nmap
...
ATTACK_SCRIPT="${SCRIPT_DIR}/../redis_unauth_lab/redis_attack.py"

if [[ -f "${ATTACK_SCRIPT}" ]]; then
    info "Copying attack script to ${REAL_HOME}/redis_attack.py"
    cp "${ATTACK_SCRIPT}" "${REAL_HOME}/redis_attack.py"
    chown "${REAL_USER}:${REAL_USER}" "${REAL_HOME}/redis_attack.py"
    chmod +x "${REAL_HOME}/redis_attack.py"
    ok "Attack script copied"
else
    warn "Attack script not found at ${ATTACK_SCRIPT}"
    info "You can manually copy redis_unauth_lab/redis_attack.py to the attacker"
fi
...
echo -e "    ${YELLOW}cd ~ && python3 redis_attack.py${NC}"
echo ""
