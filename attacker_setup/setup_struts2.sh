#!/usr/bin/env bash
# ============================================================================
# Struts2 RCE Lab — Attacker Machine Setup
# ============================================================================
# Installs all dependencies needed to run struts2_attack.py on a minimal
# Kali image (SPHERE cyber range).
#
# Usage:  sudo bash setup_struts2.sh
# ============================================================================
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  Struts2 RCE Lab — Attacker Machine Setup            ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
PACKAGES=(
    ...
    python3-pip
    python3-venv
    # HTTP client (Struts2 exploit is pure HTTP/curl based)
    curl
    # Port scanner
    nmap
...
ATTACK_SCRIPT="${SCRIPT_DIR}/../struts2_lab/struts2_attack.py"

if [[ -f "${ATTACK_SCRIPT}" ]]; then
    info "Copying attack script to ${REAL_HOME}/struts2_attack.py"
    cp "${ATTACK_SCRIPT}" "${REAL_HOME}/struts2_attack.py"
    chown "${REAL_USER}:${REAL_USER}" "${REAL_HOME}/struts2_attack.py"
    chmod +x "${REAL_HOME}/struts2_attack.py"
    ok "Attack script copied"
else
    warn "Attack script not found at ${ATTACK_SCRIPT}"
    info "You can manually copy struts2_lab/struts2_attack.py to the attacker"
fi
...
echo -e "    ${YELLOW}cd ~ && python3 struts2_attack.py${NC}"
echo ""
